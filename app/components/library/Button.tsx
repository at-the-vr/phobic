...
export const Button: ButtonComponent = ({ variant = 'primary', colors = {}, ...props }) => {
	...
	const baseClasses: string =
		'inline-flex items-center justify-center gap-2 rounded-md px-4 text-sm font-medium transition-all focus:outline-none focus:ring-2 focus:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 dark:focus:ring-offset-zinc-800 transition-colors'
	...
	const generateVariantClasses = (variant: string, colorProps: any) => {
		switch (variant) {
			...
			case 'secondary':
				return `border-2 border-gray-200 py-[0.688rem] hover:text-white ${colorProps.text || 'text-emerald-500'} ${colorProps.hoverBorder || 'hover:border-emerald-500'} ${colorProps.hoverBg || 'hover:bg-emerald-500'} ${colorProps.focusRing || 'focus:ring-emerald-500'} dark:border-zinc-700 ${colorProps.darkHoverBorder || 'dark:hover:border-emerald-500'}`
			case 'tertiary':
				return `border border-transparent py-3 underline-offset-4 ring-offset-white hover:underline ${colorProps.text || 'text-emerald-500'} ${colorProps.hoverText || 'hover:text-emerald-700'}  ${colorProps.focusRing || 'focus:ring-emerald-500'} ${colorProps.darkHoverText || 'dark:hover:text-emerald-500'}`
			default:
				return ''
		}
	}
